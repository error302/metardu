...
async function workbookRows(worksheet) {
  const rows = [];
...
for (const f of files) {
  ...
  const ccSheet = wb.getWorksheet('CONSISTENCY CHECKS');
  if (ccSheet) {
    const data = await workbookRows(ccSheet);
  ...
  if (areaSheet) {
    const data = await workbookRows(areaSheet);
    data.forEach(row => {
  ...
  for (const sheet of wb.worksheets) {
    ...
    if (name.toLowerCase().includes('level') || name.toLowerCase().includes('height')) {
       const data = await workbookRows(sheet);
