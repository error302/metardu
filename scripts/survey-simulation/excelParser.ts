...
export async function parseSurveyExcel(filePath: string): Promise<ParsedExcelData> {
  try {
...
function normalizeCellValue(cell: unknown): string | number | boolean | null {
  if (cell == null) return null;
  if (typeof cell === 'string' || typeof cell === 'number' || typeof cell === 'boolean') return cell;
  if (cell instanceof Date) return cell.toISOString();
  if (typeof cell === 'object') {
    const value = cell as { result?: unknown; text?: unknown; richText?: Array<{ text?: string }> };
