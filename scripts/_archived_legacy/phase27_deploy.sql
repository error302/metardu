-- ============================================================
-- Phase 27 Deploy: Traverse Tables + History + Versioning
-- Combined migration script for deployment
...
-- 4. Traverse versioning column (safe to re-run)
ALTER TABLE parcel_traverses ADD COLUMN IF NOT EXISTS version INT DEFAULT 1;

-- 5. Traverse history table to archive previous traverse computations
CREATE TABLE IF NOT EXISTS traverse_history (
  id SERIAL PRIMARY KEY,
  parcel_traverse_id INT REFERENCES parcel_traverses(id) ON DELETE CASCADE,
  version INT NOT NULL,
  computed_at TIMESTAMPTZ DEFAULT NOW(),
  opening_station TEXT,
  closing_station TEXT,
  opening_easting NUMERIC(12,3),
  opening_northing NUMERIC(12,3),
  closing_easting NUMERIC(12,3),
  closing_northing NUMERIC(12,3),
  is_closed BOOLEAN DEFAULT FALSE,
  perimeter NUMERIC(12,3),
  linear_error NUMERIC(12,6),
  precision_ratio NUMERIC(12,6),
  accuracy_order TEXT,
  computed_area_ha NUMERIC(12,6),
  observations JSONB,
  coordinates JSONB,
...
CREATE INDEX IF NOT EXISTS idx_traverse_history_parcel ON traverse_history(parcel_traverse_id);
