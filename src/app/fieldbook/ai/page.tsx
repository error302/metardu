...
export default function FieldBookAIPage() {
  ...
  const handleInterpret = async () => {
    ...
    try {
      ...
      if (inputMethod === 'text') {
        ...
        // ── Real AI vision interpretation ──
        // Convert the uploaded file to base64 and send to the NVIDIA vision model
        const reader = new FileReader();
        const base64Promise = new Promise<string>((resolve, reject) => {
          reader.onload = () => {
            const result = reader.result as string;
            // Strip the data:xxx;base64, prefix — we only need the raw base64
        ...
        if (!response.ok) {
          ...
          if (response.status === 403) {
            setError('AI call limit reached for your plan. Upgrade to Pro for more calls.');
          } else {
            setError(errData.error || `AI service error (${response.status})`);
          }
          setLoading(false);
          return;
        }
        ...
        if (aiResult.observations?.length > 0) {
          setResult({
            survey_type: aiResult.surveyType || 'unknown',
            stations: aiResult.observations.map((obs: any) => ({
              from: obs.from || '',
              to: obs.to || '',
              bearing: obs.bearing || '',
            ...
            warnings: [
              ...(aiResult.warnings || []),
              'AI-interpreted data must be verified by a licensed surveyor before submission.',
            ],
          });
        } else {
          // AI could not extract observations
          setResult({
            survey_type: 'unknown',
            stations: [],
            warnings: [
              ...(aiResult.warnings || []),
              'Could not extract survey observations from this image. Try a clearer photo or use text input instead.',
            ],
          });
        }
      }
    } catch (e) {
      console.error('[FieldBookAI] Interpretation failed:', e);
      setError(e instanceof Error ? e.message : 'Failed to interpret notes');
    }
  ...
  const handleSaveToProject = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? `Failed to save to project: ${err.message}` : 'Failed to save to project')
    }
    setSaving(false);
  };
            ...
            {inputMethod === 'text' ? (
              ...
              <div className="border-2 border-dashed border-[var(--border-color)] rounded-lg p-8 text-center">
                ...
                <p className="text-xs text-[var(--text-muted)] mt-4 text-center">
                  Upload a photo of your field book — AI will interpret observations using NVIDIA vision
                </p>
              </div>
            )}
