...
export default function SuperelevationPage() {
  ...
  // AUDIT FIX (2026-07-05): Result only updates when user clicks Compute
  const [computed, setComputed] = useState<{
    designSpeed: number; radius: number; roadClass: string; numLanes: number; laneWidth: number
  } | null>(null)

  const compute = () => {
    setComputed({ designSpeed, radius, roadClass, numLanes, laneWidth })
  }

  const input: SuperelevationInput = {
    designSpeed: computed?.designSpeed ?? designSpeed,
    radius: computed?.radius ?? radius,
    roadClass: computed?.roadClass ?? roadClass,
    numLanes: computed?.numLanes ?? numLanes,
    laneWidth: computed?.laneWidth ?? laneWidth,
  }
        ...
        <div className="space-y-6">
          ...
          {computed && (
            <>
              ...
              <div className="p-4 bg-zinc-900 rounded-lg border border-zinc-700">
                ...
                <div className="text-xs text-zinc-500 mt-1">
                  Ls = e × w × V / (3.6 × p), where w = {computed.laneWidth}×{computed.numLanes} = {(computed.laneWidth * computed.numLanes).toFixed(1)}m
                </div>
              </div>
          ...
          {!computed && (
            <div className="p-6 bg-zinc-900 rounded-lg border border-zinc-700 text-center text-sm text-zinc-500">
              Click <span className="text-[var(--accent)] font-semibold">Compute Superelevation</span> to calculate design superelevation rate, transition length, and RDM 1.3 compliance.
            </div>
          )}
        </div>
