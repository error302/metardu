  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-12 md:py-20">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
            <div className="max-w-2xl">
              ...
              <h1 className="text-3xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight leading-tight">
                East Africa{' '}
              ...
              <p className="mt-4 text-base md:text-lg text-[var(--text-secondary)] leading-relaxed max-w-xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-[var(--accent)] text-white hover:opacity-90 transition-opacity shadow-lg shadow-[var(--accent)]/20"
                >
            ...
            <div className="flex flex-col items-center md:items-end gap-2">
              ...
              <div className="text-5xl md:text-6xl font-bold text-[var(--text-primary)] tabular-nums">
      ...
      <section className="max-w-7xl mx-auto px-4 -mt-1">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={stat.label}
              className={`relative overflow-hidden rounded-2xl border border-[var(--border-color)] bg-gradient-to-br ${stat.gradient} p-5 transition-all duration-300 hover:border-[var(--accent)]/20`}
            >
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-red-400 uppercase tracking-wider">Urgent</h3>
                <span className="ml-auto text-[10px] font-semibold text-red-400/60 bg-red-500/10 px-2 py-0.5 rounded-full">{urgentReviews.length}</span>
              </div>
              <div className="space-y-3">
                {urgentReviews.map((item, i) => (
                  <Link
                    key={i}
                    href={item.href}
                    className="block rounded-xl border border-red-500/15 bg-[var(--bg-card)] p-3 hover:border-red-500/30 transition-all duration-200 hover:translate-x-1"
                  >
                    <p className="text-xs font-semibold text-[var(--text-primary)] truncate">{item.title}</p>
                    <p className="text-[10px] text-[var(--text-muted)] mt-1">{item.meta}</p>
                  </Link>
                ))}
                {urgentReviews.length === 0 && (
                  <p className="text-[10px] text-[var(--text-muted)] italic text-center py-4">No urgent items</p>
                )}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-sky-500/20 bg-sky-500/5 p-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-sky-400 uppercase tracking-wider">In Review</h3>
                <span className="ml-auto text-[10px] font-semibold text-sky-400/60 bg-sky-500/10 px-2 py-0.5 rounded-full">{inReviewItems.length}</span>
              </div>
              <div className="space-y-3">
                {inReviewItems.map((item, i) => (
                  <Link
                    key={i}
                    href={item.href}
                    className="block rounded-xl border border-sky-500/15 bg-[var(--bg-card)] p-3 hover:border-sky-500/30 transition-all duration-200 hover:translate-x-1"
                  >
                    <p className="text-xs font-semibold text-[var(--text-primary)] truncate">{item.title}</p>
                    <p className="text-[10px] text-[var(--text-muted)] mt-1">{item.meta}</p>
                    {item.count > 0 && (
                      <span className="inline-flex items-center gap-1 mt-2 text-[9px] text-sky-400 bg-sky-500/10 px-2 py-0.5 rounded-full">
                        {item.count} reviewers
                      </span>
                    )}
                  </Link>
                ))}
                {inReviewItems.length === 0 && (
                  <p className="text-[10px] text-[var(--text-muted)] italic text-center py-4">No items in review</p>
                )}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-amber-400 uppercase tracking-wider">New</h3>
                <span className="ml-auto text-[10px] font-semibold text-amber-400/60 bg-amber-500/10 px-2 py-0.5 rounded-full">{newReviews.length}</span>
              </div>
              <div className="space-y-3">
                {newReviews.map((item, i) => (
                  <Link
                    key={i}
                    href={item.href}
                    className="block rounded-xl border border-amber-500/15 bg-[var(--bg-card)] p-3 hover:border-amber-500/30 transition-all duration-200 hover:translate-x-1"
                  >
                    <p className="text-xs font-semibold text-[var(--text-primary)] truncate">{item.title}</p>
                    <p className="text-[10px] text-[var(--text-muted)] mt-1">{item.meta}</p>
                  </Link>
                ...
                {newReviews.length === 0 && (
                  <p className="text-[10px] text-[var(--text-muted)] italic text-center py-4">No new items</p>
                )}
              </div>
            </div>
          </div>
        ...
        {/* ── Two-Column: Discussions + CPD/Equipment ── */}
        <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
          <div className="space-y-8">
            {/* ── Discussions (Modern Forum) ── */}
            <section>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <ChatBubbleIcon className="w-5 h-5 text-[var(--accent)]" />
                  <h2 className="text-xl font-bold text-[var(--text-primary)]">Discussions</h2>
                </div>
                <Link href="/community/discussions" className="text-xs text-[var(--accent)] hover:underline flex items-center gap-1 font-medium">
              ...
              <div className="space-y-3">
                {discussionTopics.map((item, i) => (
                  <Link
                    ...
                    className="group flex items-start gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-4 hover:border-[var(--accent)]/20 transition-all duration-300"
                  >
                    {/* Icon area */}
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${
                      ...
                      {item.pinned ? (
                        ...
                        <ChatBubbleIcon className="w-4 h-4 text-[var(--text-muted)]" />
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {item.hot && (
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold bg-red-500/10 text-red-400 border border-red-500/20">
                            <HotIcon className="w-2.5 h-2.5" /> HOT
                          </span>
                        )}
                        {item.pinned && (
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-semibold border ${categoryColors[item.category] ?? 'bg-gray-500/15 text-gray-400 border-gray-500/25'}`}>
                          {item.category}
                        </span>
                        <span className="flex items-center gap-1 text-[11px] text-[var(--text-muted)]">
                          <ChatBubbleIcon className="w-3 h-3" /> {item.replies} replies
                        </span>
                      </div>
                    </div>

                    <ChevronRight className="w-4 h-4 text-[var(--text-muted)] shrink-0 mt-2 group-hover:text-[var(--accent)] transition-colors" />
                  </Link>
                ))}
              </div>
            </section>
          ...
          <aside className="space-y-6">
            ...
            <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-5">
              ...
              <div className="space-y-3 text-sm">
                {[
                  { label: 'Open reviews', value: loading ? '—' : reviewCount },
        ...
        <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 border-t border-[var(--border-color)] pt-8">
          {[
            ...
            },
            {
              ...
              text: 'CPD logs and peer-review certificates stay attached to your work automatically.',
            },
            {
