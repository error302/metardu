...
export function adjustLevelNetwork(
  ...
  for (const obs of observations) {
    ...
    // to station
    if (freeIndex.has(obs.toId)) {
      row[freeIndex.get(obs.toId)] = 1
    }
    // from station
    if (freeIndex.has(obs.fromId)) {
      row[freeIndex.get(obs.fromId)] = -1
    }

    const computedDiff = heights.get(obs.toId) - heights.get(obs.fromId)
