...
export function renderTopographicPlan(opts: TopographicPlanOptions): string {
  ...
  // ⚡ Bolt Optimization: Use a single pass loop instead of Math.min/max with spread operators
  // Impact: Prevents V8 'Maximum call stack size exceeded' on large spot height arrays
  // and eliminates 4 intermediate array allocations
