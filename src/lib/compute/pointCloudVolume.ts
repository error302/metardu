...
export function gridMethodVolume(
  ...
  // ⚡ Bolt Optimization: Avoid spread operator on potentially large point cloud arrays
...
function getBounds(points: Point3D[]) {
  // ⚡ Bolt Optimization: Avoid spread operator to prevent call stack issues on large arrays
  let minE = Infinity, maxE = -Infinity, minN = Infinity, maxN = -Infinity
