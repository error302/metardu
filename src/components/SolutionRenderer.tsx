        ...
        <Section title="Given">
          <div className="grid md:grid-cols-2 gap-2">
            {solution.given.map((g, i) => (
              <div key={i} className="flex items-baseline justify-between gap-3 p-2 rounded bg-gray-950/30 border border-gray-800">
                <div className="text-xs text-[var(--text-muted)]">{g.label}</div>
                <div className="font-mono text-sm text-gray-200 text-right">{g.value}</div>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section title="Solution">
          <div className="space-y-3">
            {solution.solution.map((s, i) => (
              <div key={i} className="rounded-lg border border-gray-800 bg-gray-950/20 p-3">
            ...
            <div className="space-y-2">
              {solution.check.map((c, i) => (
                ...
                >
                  ...
                  <div className="font-mono text-sm text-gray-200 text-right">{c.value}</div>
                </div>
              ))}
            </div>
          </Section>
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="grid grid-cols-[100px_1fr] gap-3 text-sm">
      ...
      <div className={`font-mono ${highlight ? 'text-amber-200 font-semibold' : 'text-gray-200'}`}>{value}</div>
    </div>
  )
}
