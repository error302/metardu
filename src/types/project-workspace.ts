...
function getLevellingSteps(project: MetarduProject): WorkspaceStep[] {
  ...
  return [
    ...
    { id: 'closure', label: 'Closure Check', description: `Allowed: 10√K mm${ld?.closurePassed === true ? ' —  PASSED' : ld?.closurePassed === false ? ' — [x] FAILED' : ''}`, status: ld?.closurePassed !== undefined ? (ld.closurePassed ? 'complete' : 'in_progress') : 'pending', gated: true },
