...
-- Add geometry column (SRID 32737 = UTM Zone 37S, change for other zones)
alter table survey_points 
add column if not exists geom geometry(Point, 32737);
...
begin
  ...
  if new.easting is not null and new.northing is not null then
    ...
    -- Note: Using UTM Zone 37S (32737) - adjust for your zone
    new.geom = ST_SetSRID(
...
set geom = ST_SetSRID(ST_MakePoint(easting, northing), 32737)
...
-- WHERE ST_DWithin(geom, ST_SetSRID(ST_MakePoint(500000, 4500000), 32737), 100);

-- Find nearest point to a given location
-- SELECT * FROM survey_points 
-- ORDER BY geom <-> ST_SetSRID(ST_MakePoint(500000, 4500000), 32737) 
-- LIMIT 1;

-- Find points within a bounding box
-- SELECT * FROM survey_points 
-- WHERE geom && ST_MakeEnvelope(499000, 4499000, 501000, 4501000, 32737);
